...
#         pass
    ...
    def setTime(self,time):
        #FADCS=[16000000/2**x for x in range(1,8)].reverse()
        for i in reversed(range(1,8)):
            #FADC=16000000/2**i
            #us=1894.66 *(-(10920./FADC) + 1.* time/1000) 
    ...
    def run(self):
        ...
        def reqdata(parent=None): 
            #print "req data"
            p.queue.ev.set()

        framecallbacks['buffserial']=reqdata
        terminatehooks.append(reqdata)
...
class SerialWorker(Process):
    def __init__(self,port,baud,fnstr=None):
        ...
        self.queue=BufferedPipedQueue()#Queue()
        self.cmdq=Queue()
        self.port=port
        self.baud=baud
        self.devstate={}
    ...
    #                         self.fns.append(lambda x: x[i])
                ...
                data={'trigger':trigger,'values':vals,'raw':repr(str(self.mode)+'-'+s0)}
...
if __name__=='__main__':
    ...
    q=SharedQueue(buffsize=1000000)
    ...
    p.join()    
    print '---------------------------------------'
    for q in [SharedQueue(buffsize=1000000),
              MPQ(),
              PipedQueue()]:
        print "Testing", q
